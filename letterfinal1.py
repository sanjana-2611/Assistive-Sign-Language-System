...
def call_ollama(words):
    ...
    Rules enforced in prompt:
      - Every word in `words` MUST appear in the sentence (exact form or natural
        grammatical variant e.g. plural/tense is fine).
      - NO extra topic words, filler phrases, or encouragement words added.
      - Sentence must be short (max 12 words), natural, grammatically correct.

    Example:
      words = ["cat", "run"]  →  "The cat runs fast every day."
      words = ["al", "nov"]   →  "Al visits in November."   ← treats as given
    """
    words = [w.strip().lower() for w in words if w.strip()]
    if not words:
        return ""

    word_list_display = ", ".join(f'"{w}"' for w in words)
    word_list_plain   = " | ".join(words)

    prompt = (
        f"Words to use: {word_list_display}\n\n"
        f"Write ONE short English sentence (maximum 10 words) that:\n"
        f"  1. Contains EVERY one of these words: {word_list_plain}\n"
        f"  2. Does NOT add any other meaningful content words "
        f"     (articles like 'the/a', prepositions, and 'is/are/was' are fine).\n"
        f"  3. Is grammatically correct and makes sense.\n"
        f"  4. Does NOT include encouragement phrases like "
        f"     'you're doing great', 'keep it up', 'well done', etc.\n\n"
        f"Return ONLY the sentence — no explanation, no extra text."
    )
    ...
    try:
                ...
                "messages": [
                    {
                        ...
                        "content": (
                            ...
                            "Never omit any of the given words."
                        ),
                    },
                    {"role": "user", "content": prompt},
                ],
        ...
        data = resp.json()
        if "message" in data:
            sentence = data["message"]["content"].strip()
            # Strip any accidental prefix like "Sentence: ..."
            for prefix in ("sentence:", "answer:", "output:", "result:"):
                if sentence.lower().startswith(prefix):
                    sentence = sentence[len(prefix):].strip()
            return sentence
    except Exception as exc:
        print(f"[Ollama sentence] Error: {exc}")
